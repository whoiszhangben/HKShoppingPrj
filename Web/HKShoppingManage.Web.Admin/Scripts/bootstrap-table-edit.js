...
(function ($) {
    ...
    function initTrClick(that, _this) {
        ...
        $td.html('<div style="margin:0;padding:0;overflow:hidden;border:solid 0px red;height:' + (height) + 'px;width:' + (width) + 'px;">'
            ...
            + '</div>');
        $td.width(width);
        if ($td.hasClass("priceCls")) {
            var $input = $td.find('input');
            if (!tdOpt.edit.type || tdOpt.edit.type == 'text') {
                $input.unbind('blur').on('blur', function (event) {
                    var $qtyinput = $tr.children("td.qtyCls").find("input");
                    var $exrateinput = $tr.children("td.exRateCls").find("input");
                    var $amountinput = $tr.children("td.amountCls").find("input");
                    var $amountrmbinput = $tr.children("td.amountRMBCls").find("input");
                    $amountinput.val(parseFloat(!!$input.val() ? $input.val() : 0) * parseFloat(!!$qtyinput.val() ? $qtyinput.val() : 0));
                    $amountrmbinput.val(parseFloat(!!$amountinput.val() ? $amountinput.val() : 0) * parseFloat(!!$exrateinput.val() ? $exrateinput.val() : 0));
                });

            }
        } else if ($td.hasClass("exRateCls")) {
            var $input = $td.find('input');
            if (!tdOpt.edit.type || tdOpt.edit.type == 'text') {
                $input.unbind('blur').on('blur', function (event) {
                    var $qtyinput = $tr.children("td.qtyCls").find("input");
                    var $priceinput = $tr.children("td.priceCls").find("input");
                    var $amountinput = $tr.children("td.amountCls").find("input");
                    var $amountrmbinput = $tr.children("td.amountRMBCls").find("input");
                    $amountinput.val(parseFloat(!!$qtyinput.val() ? $qtyinput.val() : 0) * parseFloat(!!$priceinput.val() ? $priceinput.val() : 0));
                    $amountrmbinput.val(parseFloat(!!$amountinput.val() ? $amountinput.val() : 0) * parseFloat(!!$input.val() ? $input.val() : 0));
                });
            }
        } else if ($td.hasClass("qtyCls")) {
            var $input = $td.find('input');
            if (!tdOpt.edit.type || tdOpt.edit.type == 'text') {
                $input.unbind('blur').on('blur', function (event) {
                    var $exrateinput = $tr.children("td.exRateCls").find("input");
                    ...
                    var $amountinput = $tr.children("td.amountCls").find("input");
                    var $amountrmbinput = $tr.children("td.amountRMBCls").find("input");
                    $amountinput.val(parseFloat(!!$input.val() ? $input.val() : 0) * parseFloat(!!$priceinput.val() ? $priceinput.val() : 0));
                    $amountrmbinput.val(parseFloat(!!$amountinput.val() ? $amountinput.val() : 0) * parseFloat(!!$exrateinput.val() ? $exrateinput.val() : 0));
                });
            }
        } else {
